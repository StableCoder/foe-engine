...
TEST_CASE("foeImexBinaryResult - Ensure error codes return correct values and strings") {
    ...
    SECTION("Generic non-existant negative value") {
        ...
        CHECK(std::string{resultString} == "FOE_IMEX_BINARY_UNKNOWN_ERROR_2147483647");
    }
    SECTION("Generic non-existant positive value") {
        foeImexBinaryResultToString((foeImexBinaryResult)FOE_RESULT_MAX_ENUM, resultString);
        CHECK(std::string{resultString} == "FOE_IMEX_BINARY_UNKNOWN_SUCCESS_2147483647");
    }
    ...
    ERROR_CODE_CATCH_CHECK(FOE_IMEX_BINARY_ERROR_FAILED_TO_OPEN_FILE)
    ERROR_CODE_CATCH_CHECK(FOE_IMEX_BINARY_ERROR_FAILED_TO_FIND_RESOURCE_DATA)
    ERROR_CODE_CATCH_CHECK(FOE_IMEX_BINARY_ERROR_FAILED_TO_FIND_EXTERNAL_DATA)
}
