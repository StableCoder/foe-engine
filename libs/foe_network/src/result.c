...
void foeNetworkResultToString(foeNetworkResult value, char buffer[FOE_MAX_RESULT_STRING_SIZE]) {
    ...
    switch (value) {
        ...
        RESULT_CASE(FOE_NETWORK_NO_DATA_READ)
        ...
        RESULT_CASE(FOE_NETWORK_ERROR_SOCKET_FAILED_TO_SET_NONBLOCKING)
    ...
    default:
        if (value > 0) {
            snprintf(buffer, FOE_MAX_RESULT_STRING_SIZE, "FOE_NETWORK_UNKNOWN_SUCCESS_%i", value);
        } else {
            value = abs(value);
            snprintf(buffer, FOE_MAX_RESULT_STRING_SIZE, "FOE_NETWORK_UNKNOWN_ERROR_%i", value);
        }
    }
