...
TEST_CASE("AES 256 GCM") {
    if (foeCrypto_AES_256_GCM_isHardwareAccelerated()) {
        SECTION("encryption success cases") {
            ...
            std::unique_ptr<unsigned char[]> encryptedBuffer(
                new unsigned char[encryptedBufferSize + 16]);
        ...
        SECTION("decryption success cases") {
            foeCryptoKey key = FOE_NULL_HANDLE;
            foeCryptoContext_AES_256_GCM context = FOE_NULL_HANDLE;
            foeResultSet result;
            ...
            SECTION("destination buffer size is exact required") {
                ...
                REQUIRE(result.value == FOE_SUCCESS);
                REQUIRE(decryptedDataSize == decryptedDataBufferSize);
                REQUIRE(memcmp(cOriginalData.data(), decryptedBuffer.get(), cOriginalData.size()) ==
                        0);
            }
            SECTION("destination buffer size is too large") {
        ...
        SECTION("encryption context creation failure cases") {
            foeCryptoKey key = FOE_NULL_HANDLE;
            foeCryptoContext_AES_256_GCM context = FOE_NULL_HANDLE;
            foeResultSet result;

            SECTION("key too small") {
                result = foeCreateCryptoKey(FOE_CRYPTO_AES_256_GCM_KEY_SIZE - 1,
                                            invalidKeyData.data(), &key);
                REQUIRE(result.value == FOE_SUCCESS);
        ...
        SECTION("encryption failure cases") {
            foeCryptoKey key = FOE_NULL_HANDLE;
            foeCryptoContext_AES_256_GCM context = FOE_NULL_HANDLE;
            foeResultSet result;
            ...
            SECTION("invalid nonce sizes") {
                size_t encryptedBufferSize =
                    cOriginalData.size() + FOE_CRYPTO_AES_256_GCM_ENCRYPTION_OVERHEAD;
                std::unique_ptr<unsigned char[]> encryptedBuffer(
                    new unsigned char[encryptedBufferSize]);
                memset(encryptedBuffer.get(), 0, encryptedBufferSize);

                SECTION("too small") {
                    size_t encryptedDataSize = encryptedBufferSize;
                    result = foeCryptoEncrypt_AES_256_GCM(
                        context, nonce1.size() - 1, nonce1.data(), cOriginalData.size(),
        ...
        SECTION("decryption failure cases") {
            foeCryptoKey key = FOE_NULL_HANDLE;
            foeCryptoContext_AES_256_GCM context = FOE_NULL_HANDLE;
            foeResultSet result;
            ...
            SECTION("any byte in encrypted buffer is modified") {
                for (size_t i = 0; i < encryptedDataSize; ++i) {
                    ...
                    size_t decryptedDataSize = decryptedDataBufferSize;
                    result = foeCryptoDecrypt_AES_256_GCM(
                        context, nonce1.size(), nonce1.data(), encryptedDataSize,
                        encryptedBufferCopy.get(), &decryptedDataSize, decryptedBuffer.get());
        ...
        SECTION("no AES hardware acceleration detected means context cannot be created") {
            foeCryptoKey key = FOE_NULL_HANDLE;
            foeCryptoContext_AES_256_GCM context = FOE_NULL_HANDLE;
            foeResultSet result;
