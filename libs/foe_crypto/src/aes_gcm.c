...
_Static_assert(FOE_CRYPTO_AES_GCM_KEY_SIZE == crypto_aead_aes256gcm_KEYBYTES,
               "key size is incorrect");

_Static_assert(FOE_CRYPTO_AES_GCM_NONCE_SIZE == crypto_aead_aes256gcm_NPUBBYTES,
               "nonce size is incorrect");

_Static_assert(FOE_CRYPTO_AES_GCM_ENCRYPTION_OVERHEAD == crypto_aead_aes256gcm_ABYTES,
               "overhead size is incorrect");

FOE_DEFINE_HANDLE_CASTS(encryption_state, crypto_aead_aes256gcm_state, foeCryptoContext_AES_GCM)
...
foeResultSet foeCreateContext_AES_GCM(foeCryptoKey key, foeCryptoContext_AES_GCM *pContext) {
    ...
    if (foeCryptoGetKeySize(key) != FOE_CRYPTO_AES_GCM_KEY_SIZE)
...
foeResultSet foeCryptoDecrypt_AES_GCM(foeCryptoContext_AES_GCM context,
    ...
    if (encryptedDataSize > *pDecryptedDataSize + FOE_CRYPTO_AES_GCM_ENCRYPTION_OVERHEAD)
        return to_foeResult(FOE_CRYPTO_ERROR_DESTINATION_BUFFER_TOO_SMALL);
