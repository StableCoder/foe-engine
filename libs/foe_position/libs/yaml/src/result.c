...
void foePositionYamlResultToString(foePositionYamlResult value,
    ...
    default:
        if (value > 0) {
            snprintf(buffer, FOE_MAX_RESULT_STRING_SIZE, "FOE_POSITION_YAML_UNKNOWN_SUCCESS_%i",
                     value);
        } else {
            value = abs(value);
            snprintf(buffer, FOE_MAX_RESULT_STRING_SIZE, "FOE_POSITION_YAML_UNKNOWN_ERROR_%i",
                     value);
        }
    }
