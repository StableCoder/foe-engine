 ...
 */
FOE_GFX_EXPORT
uint32_t foeGfxVkMipmapCount(VkExtent3D extent);
 ...
 */
FOE_GFX_EXPORT
VkExtent3D foeGfxVkMipmapExtent(VkExtent3D extent, uint32_t mipLevel);
 ...
 */
FOE_GFX_EXPORT
VkDeviceSize foeGfxVkExtentPixelCount(VkExtent3D extent, uint32_t mipLevels);

FOE_GFX_EXPORT
foeResultSet foeGfxVkRecordImageUploadBufferUploadCommands(
...
FOE_GFX_EXPORT
foeResultSet foeGfxVkRecordImageBufferUploadCommands(
    foeGfxUploadContext uploadContext,
    VkImageSubresourceRange const *pSubresourceRange,
    uint32_t copyRegionCount,
    VkBufferImageCopy const *pCopyRegions,
...
FOE_GFX_EXPORT
VkImageAspectFlags foeGfxVkFormatAspects(VkFormat format);
