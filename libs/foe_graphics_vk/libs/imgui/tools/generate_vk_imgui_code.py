...
def main(argv):
    ...
    # Enum Processing
    headerFile = open(
        '../include/foe/graphics/vk/imgui/display_vk_enums.hpp', 'w')
    sourceFile = open('../src/display_vk_enums.cpp', 'w')
...
""".format(2022))
    ...
    for enum in xmlRoot.findall('enums/'):
        headerFile.write('\n')
        sourceFile.write('\n')

        # Header
        guarded = guardItem(enum, firstVersion, lastVersion, headerFile)
        headerFile.write(
            'FOE_GFX_VK_IMGUI_EXPORT void imgui_{0}(std::string const &label, {0} const &data);\n'.format(enum.tag.replace('FlagBits', 'Flags')))
        if guarded:
            headerFile.write('#endif\n')

        # Source
        guarded = guardItem(enum, firstVersion, lastVersion, sourceFile)
        sourceFile.write("""void imgui_{0}(std::string const &label, {0} const &data) {{
    std::string serializedStr;
    vk_serialize("{0}", data, &serializedStr);
...
""")
    ...
    # Struct Processing
    headerFile = open(
        '../include/foe/graphics/vk/imgui/display_vk_structs.hpp', 'w')
    sourceFile = open('../src/display_vk_structs.cpp', 'w')
...
""".format(structName))
        ...
        for member in struct.findall('members/'):
            ...
            if memberTypeSuffix and '*' in memberTypeSuffix and member.get('len'):
                ...
                sourceFile.write("""
    // {0} - {1} / {2}
    if(std::string nodeName = "{1}:" + std::to_string(data.{2}) + " ({0})";
