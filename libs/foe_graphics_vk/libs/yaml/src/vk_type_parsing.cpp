...
#define INSTANTIATION(T)                                                                           \
                                                                                                   \
    template <>                                                                                    \
    FOE_GFX_VK_YAML_EXPORT bool yaml_read_optional<T>(std::string const &nodeName,                 \
                                                      YAML::Node const &node, T &data) {           \
        return yaml_read_optional_vk<T>(#T, nodeName, node, data);                                 \
    }                                                                                              \
                                                                                                   \
    template <>                                                                                    \
    FOE_GFX_VK_YAML_EXPORT void yaml_read_required<T>(std::string const &nodeName,                 \
                                                      YAML::Node const &node, T &data) {           \
        yaml_read_required_vk<T>(#T, nodeName, node, data);                                        \
    }                                                                                              \
                                                                                                   \
    template <>                                                                                    \
    FOE_GFX_VK_YAML_EXPORT void yaml_write_required<T>(std::string const &nodeName, T const &data, \
                                                       YAML::Node &node) {                         \
        yaml_write_required_vk<T>(#T, nodeName, data, node);                                       \
    }                                                                                              \
                                                                                                   \
    template <>                                                                                    \
    FOE_GFX_VK_YAML_EXPORT bool yaml_write_optional<T>(                                            \
...
// INSTANTIATION(VkIndirectCommandsLayoutUsageFlagBitsNVX)
// INSTANTIATION(VkObjectEntryUsageFlagBitsNVX)
// INSTANTIATION(VkIndirectCommandsTokenTypeNVX)
// INSTANTIATION(VkObjectEntryTypeNVX)
...
INSTANTIATION(VkGeometryInstanceFlagBitsNV)
INSTANTIATION(VkGeometryFlagBitsNV)
INSTANTIATION(VkBuildAccelerationStructureFlagBitsNV)
INSTANTIATION(VkCopyAccelerationStructureModeNV)
INSTANTIATION(VkAccelerationStructureTypeNV)
INSTANTIATION(VkGeometryTypeNV)
INSTANTIATION(VkAccelerationStructureMemoryRequirementsTypeNV)
INSTANTIATION(VkRayTracingShaderGroupTypeNV)
