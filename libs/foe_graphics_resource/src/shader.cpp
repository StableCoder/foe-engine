...
foeShader::foeShader(foeResourceID resource, foeResourceFns const *pResourceFns) :
    foeResourceBase{resource, pResourceFns} {}

foeShader::~foeShader() {
    if (useCount > 0) {
        FOE_LOG(foeGraphicsResource, Warning,
                "foeShader {} being destroyed despite having active uses", foeIdToString(resource));
    }
    if (refCount > 0) {
        FOE_LOG(foeGraphicsResource, Warning,
...
void foeShader::loadCreateInfo() {
    ...
    if (!isLoading.compare_exchange_strong(expected, true)) {
        ...
        FOE_LOG(foeGraphicsResource, Warning, "Attempted to load foeShader {} in parrallel",
                foeIdToString(resource))
        decrementRefCount();
        return;
    }
    ...
    if (pResourceFns->asyncTaskFn) {
        FOE_LOG(foeGraphicsResource, Verbose, "Creating foeShader {} asynchronously",
                foeIdToString(resource))
        pResourceFns->asyncTaskFn(createFn);
    } else {
        FOE_LOG(foeGraphicsResource, Verbose, "Creating foeShader {} synchronously",
                foeIdToString(resource))
        createFn();
    }
}
...
void postLoadFn(void *pResource, std::error_code errC) {
    auto *pImage = reinterpret_cast<foeShader *>(pResource);
...
void foeShader::loadResource(bool refreshCreateInfo) {
    incrementRefCount();
    ...
    if (pResourceFns->asyncTaskFn) {
        FOE_LOG(foeGraphicsResource, Verbose, "Loading foeShader {} asynchronously",
                foeIdToString(resource))
        pResourceFns->asyncTaskFn(loadFn);
    } else {
        FOE_LOG(foeGraphicsResource, Verbose, "Loading foeShader {} synchronously",
                foeIdToString(resource))
        loadFn();
    }
}

void foeShader::unloadResource() {
    modifySync.lock();
