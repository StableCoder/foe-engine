...
foeResultSet export_foeMeshFileCreateInfo(foeResourceCreateInfo createInfo,
    ...
    if (ciType == FOE_GRAPHICS_RESOURCE_STRUCTURE_TYPE_MESH_FILE_CREATE_INFO) {
        foeMeshFileCreateInfo const *pData = foeResourceCreateInfoGetData(createInfo);

        result = binary_write_foeMeshFileCreateInfo(pData, &set.dataSize, NULL);
        ...
        result = binary_write_foeMeshFileCreateInfo(pData, &set.dataSize, set.pData);
        set.pKey = binary_key_foeMeshFileCreateInfo();
