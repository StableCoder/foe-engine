...
foeResultSet export_foeImageCreateInfo(foeResourceCreateInfo createInfo,
    ...
    if (ciType == FOE_GRAPHICS_RESOURCE_STRUCTURE_TYPE_IMAGE_CREATE_INFO) {
        foeImageCreateInfo const *pData = foeResourceCreateInfoGetData(createInfo);

        result = binary_write_foeImageCreateInfo(pData, &set.dataSize, NULL);
        ...
        result = binary_write_foeImageCreateInfo(pData, &set.dataSize, set.pData);
        set.pKey = binary_key_foeImageCreateInfo();
