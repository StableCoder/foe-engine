...
TEST_CASE("Ensure error codes return correct values and strings") {
    ...
    SECTION("Generic non-existant negative value") {
        errC = static_cast<foeWsiResult>(INT_MIN);

        CHECK(errC.value() == INT_MIN);
        CHECK(errC.message() == "(unrecognized negative foeWsiResult value)");
    }
    SECTION("Generic non-existant positive value") {
        errC = static_cast<foeWsiResult>(INT_MAX);

        CHECK(errC.value() == INT_MAX);
        CHECK(errC.message() == "(unrecognized positive foeWsiResult value)");
    }
