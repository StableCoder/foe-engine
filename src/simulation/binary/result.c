...
void foeBringupBinaryResultToString(foeBringupBinaryResult value,
    ...
    switch (value) {
        ...
        RESULT_CASE(FOE_BRINGUP_BINARY_ERROR_ARMATURE_STATE_POOL_NOT_FOUND)
        RESULT_CASE(FOE_BRINGUP_BINARY_ERROR_FAILED_TO_REGISTER_CAMERA_IMPORTER)
        RESULT_CASE(FOE_BRINGUP_BINARY_ERROR_CAMERA_POOL_NOT_FOUND)
        RESULT_CASE(FOE_BRINGUP_BINARY_ERROR_FAILED_TO_REGISTER_RENDER_STATE_IMPORTER)
        RESULT_CASE(FOE_BRINGUP_BINARY_ERROR_RENDER_STATE_POOL_NOT_FOUND)

    default:
        if (value > 0) {
            snprintf(buffer, FOE_MAX_RESULT_STRING_SIZE, "FOE_BRINGUP_BINARY_UNKNOWN_SUCCESS_%i",
                     value);
        } else {
            value = abs(value);
            snprintf(buffer, FOE_MAX_RESULT_STRING_SIZE, "FOE_BRINGUP_BINARY_UNKNOWN_ERROR_%i",
                     value);
        }
    }
