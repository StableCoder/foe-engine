...
foeResultSet onRegister(foeExporter exporter) {
    ...
    if (std::string_view{exporter.pName} == "Yaml") {
        ...
        if (result.value != FOE_SUCCESS) {
            result = to_foeResult(FOE_SKUNKWORKS_YAML_ERROR_FAILED_TO_REGISTER_RESOURCE_EXPORTERS);
        ...
        if (result.value != FOE_SUCCESS) {
            result = to_foeResult(FOE_SKUNKWORKS_YAML_ERROR_FAILED_TO_REGISTER_COMPONENT_EXPORTERS);
            goto REGISTRATION_FAILED;
        }
    }
